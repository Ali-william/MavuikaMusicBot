...
module.exports = async (client) => {
    ...
    (async () => {
        try {
            ...
            console.log(`${colors.cyan}[ USER ]${colors.reset} ${colors.yellow}Ali No-Baka${colors.reset}`);
        } catch (err) {
            console.log('\n' + '─'.repeat(40));
            console.log(`${colors.magenta}${colors.bright}⚡ COMMAND STATUS${colors.reset}`);
            console.log('─'.repeat(40));
            ...
            console.log(`${colors.cyan}[ TIME ]${colors.reset} ${colors.gray}${new Date().toISOString().replace('T', ' ').split('.')[0]}${colors.reset}`);
            console.log(`${colors.cyan}[ USER ]${colors.reset} ${colors.yellow}AliNoBaka${colors.reset}`);
        }
    })();
